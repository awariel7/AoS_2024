...
,cte_food as (
	select tm.*
		,unnest(xpath('/christmas_feast/organizational_details/menu_registry/course_details/dish_entry/food_item_id/text()', tm.menu_data)::TEXT[]) AS f_food_item_id
		,unnest(xpath('/northpole_database/annual_celebration/event_metadata/menu_items/food_category/food_category/dish/food_item_id/text()', tm.menu_data)::TEXT[]) AS f_food_item_id2
	from temp_menus as tm
)
